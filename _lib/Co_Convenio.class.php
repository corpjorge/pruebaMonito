		...
		foreach ($this->materias as $vMateria) {
			...
			if ($unTipoMonitor->minvalorhora > 0 && $unTipoMonitor->maxvalorhora > 0) {
				...
				if ($vMateria[curso] == "VICE3001" || $vMateria[curso] == "VICE3002") {
					/*						echo $this->valorhora ." - - ". $valorMonitoriaVice; 
						echo "<br>";
						echo $valorHora ." - - ". $valorMonitoriaVice; 
						exit; */
					if ($valorHora != $valorMonitoriaVice) {
						$this->validacion->acumuladoErrores['valorhora' . $i] = 'El valor de la hora no se encuentra en el rango permitido';
						$returnValue = false;
					}
				} else {
	...
	*/
	...
	function validarHorasDisponibles($codigo)
	{
		...
		if ($this->labor == 'I') {
			...
			/*
			* Cristian Arenas  05-08-2019
			* Se modificio la funcion para que diferencie el tipo de periodo, para monitorias Docencia
			*/
			$horasSemanales = array();
			foreach ($this->materias as $vMateria) {
				$tipoPeriodo = $this->cargarTipoPeriodo($vMateria['curso'], $vMateria['seccion']);
				$horasSemanales[$tipoPeriodo] += $vMateria['horassemanales'];
			...
			$returnValue = $unEstudiante->tieneHorasDisponiblesXPeriodo($this->periodoacademico, $codigo, $horasSemanales, $this->convenio, $vice);
		}
		if (!$returnValue) {
...
*/
		...
		if (empty($sesionObjetosCosto)) {
			...
			foreach ($arrayModulo as $vModulo) {
				...
				if (is_object($objetosCosto)) {
					$tmpObjectToArray = (array) $objetosCosto;
					$objetosCosto = $tmpObjectToArray;
				}
	...
	function cargarInformacionPDF($convenios = array())
	{
		...
		$directorio2 = '/datos/hypertext/monitorias.uniandes.edu.co.80/_tmp';
		$final = fopen($directorio2 . "/Ver_datos.txt", "a+");
		fwrite($final, "\n---------------");
		fwrite($final, "\nValor: " . print_r($sentencia, true));
		fclose($final);
		$informacion = array();
		$convenioActual = '';
		$i = -1;
	...
	function cargarInformacionExcel($filtros = array())
	{
		...
		$sentencia = "
			SELECT DISTINCT
              ...
              N.CODE_PARTE_PERIODO,
              N.DESC_PARTE_PERIODO
			FROM CO_ESTUDIANTE E, 
			  CO_CONVENIO C, 
			  CO_DEPENDENCIA D, 
			  CO_TIPOLABOR TL,
			...
			WHERE E.ESTUDIANTE = C.ESTUDIANTE
			  ...
			  AND PP.PLANPAGOS = PL.PLANPAGOS
			  AND C.CONVENIO = M.CONVENIO (+)
			  AND C.PERIODOACADEMICO = R.PERIODOACADEMICO
              AND R.PERIODOSEMESTRE =  N.MAT_SEMESTRE
			  $condicion
		...
		/*AGREGA PLANES DE PAGOS POR PERIODO*/
		$planPagos = array();

		while ($unInformacion->fetch()) {
			//obtiene el plan de pagos para el periodo de la monitoría, solo consulta una vez por periodo
			if (!isset($planPagos[$unInformacion->periodoacademico])) {
				$planPagos[$unInformacion->periodoacademico] = array();
				$sentenciaPagos = "SELECT 
					C.POSICION,
					C.PORCENTAJE,
					C.FECHA
					FROM CO_PLANPAGOS P,
					CO_PORCENTAJEPAGOS C
					WHERE P.PLANPAGOS = C.PLANPAGOS
					AND P.PERIODOACADEMICO = {$unInformacion->periodoacademico}";
				$unPlanPagos = new DB_DataObject();
				$unPlanPagos->query($sentenciaPagos);
				while ($unPlanPagos->fetch()) {
					$planPagos[$unInformacion->periodoacademico]["{$unPlanPagos->porcentaje}_{$unPlanPagos->posicion}"] = array(
						'posicion' => $unPlanPagos->posicion,
						'porcentaje' => $unPlanPagos->porcentaje,
						'fecha' => $unPlanPagos->fecha,
					);
				}
			}
			...
			$_pp = $planPagos[$unInformacion->periodoacademico];
			$_tipoFecha = ($unInformacion->distribucion == '50% - 50%') ? 50 : 100;
			$_mids = array('8a', '8b');
			$fechaPago1 = 'N/A';
			$fechaPago2 = 'N/A';
			if (in_array(strtolower($unInformacion->code_parte_periodo), $_mids)) {
				//si es de 8 semanas
				if (strtolower($unInformacion->code_parte_periodo) == '8a') {
					//8a
					$fechaPago1 = $_pp['50_1']['fecha'];
				} else {
					//8b
					$fechaPago2 = $_pp['50_2']['fecha'];
				}
			} elseif (strtolower($unInformacion->code_parte_periodo) != '') {
				//si es de 16 semanas
				if ($_tipoFecha == 50) {
					$fechaPago1 = $_pp['50_1']['fecha'];
					$fechaPago2 = $_pp['50_2']['fecha'];
				} else {
					$fechaPago2 = $_pp['100_1']['fecha'];
				}
			}
			...
			$informacion[$i]['materia'][] = array(
				...
				'valorhoracurso' => $unInformacion->valorhoracurso,
				'horassemanalescurso' => $unInformacion->horassemanalescurso,
				'tipoperiodo' => $unInformacion->code_parte_periodo,
				'desctipoperiodo' => $unInformacion->desc_parte_periodo,
				'fechaprimerpago' => $fechaPago1,
				'fechasegundopago' => $fechaPago2
			);
		}
		return $informacion;
	}
	...
	/**
	 * Co_Convenio::cargarTipoPeriodo()
	 * 
	 * Cristian Arenas 05-08-2019
	 * Devuelve el tipo de periodo buscando por el codigo de materia y seccion, 8A o 8B, en caso contrario le pone la marca de otro 
	 *
	 * @param $codigoMateria
	 * @param $sesion
	 * @param $periodoSemestre
	 * @return string
	 **/
	function cargarTipoPeriodo($codigoMateria, $seccion)
	{
		...
		$sentencia = "
			SELECT DISTINCT 
				CASE WHEN CODE_PARTE_PERIODO IS NULL OR 
						(CODE_PARTE_PERIODO <> '8A' AND  CODE_PARTE_PERIODO <> '8B') THEN 'OTRO' 
					ELSE CODE_PARTE_PERIODO END AS TIPO 
			FROM MATERIA@NIFE 
			WHERE MAT_SEMESTRE IN (
					SELECT PERIODOSEMESTRE 
					FROM CO_PERIODOACADEMICO 
